...
/**
 * Reads a maximum of the given <i>Size</i> in {@link Simulator#MEBIBYTE}
...
public class ReadFileTask extends TaskImpl {
	...
	public ReadFileTask(Workload workload, Config config) {
	...
	public void execute() {
		...
		file = getWorkload().getParent().getParent().getCloudProvider().getStaas().getReplicaSelectionPolicy()
				.selectBestReplica(file, StorageOperationType.READ, getWorkload().getParent().getParent());
		...
		this.operation = getWorkload().readFile(file, this.pos, this.remainingSize, new NotificationListener() {
			@Override
