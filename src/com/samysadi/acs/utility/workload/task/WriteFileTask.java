 ...
 * in the workload's active file.<br/>
 ...
 *
 * <p>You need to define the file to write, and the remote job that will write the file
 * using appropriate tasks.
 *
 * @since 1.0
 */
public class WriteFileTask extends TaskImpl {
	private StorageOperation operation;
	private long remainingSize;
	private long pos;

	public WriteFileTask(Workload workload, Config config) {
		super(workload, config);

		this.operation = null;
		this.remainingSize = FactoryUtils.generateLong("Size", getConfig(), 0l) * Simulator.MEBIBYTE;
		this.pos = FactoryUtils.generateLong("Pos", getConfig(), 0l) * Simulator.MEBIBYTE;
	}
	...
	public void execute() {
		...
		file = getWorkload().getParent().getParent().getCloudProvider().getStaas().getReplicaSelectionPolicy()
				.selectBestReplica(file, StorageOperationType.WRITE, getWorkload().getParent().getParent());
		...
		this.remainingSize = Math.min(
				file.getSize() - this.pos,
				this.remainingSize
			);
		...
		this.operation = getWorkload().writeFile(file, this.pos, this.remainingSize, new NotificationListener() {
			@Override
